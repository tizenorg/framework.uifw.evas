   ...
   if (gc->shared)
     {
        ...
        if (gc->shared->provider)
           native_buffer_provider_destroy (gc->shared->provider);
#endif
#endif
     }
                                  ...
                                  Eina_Bool smooth, Eina_Bool tex_only)
{
   ...
   if (tex->im && tex->im->native.target == GL_TEXTURE_EXTERNAL_OES)
     {
        prog = gc->shared->shader[evas_gl_common_shader_choice(0, NULL, r, g, b, a,
                                                               SHADER_TIZEN_NOMUL, SHADER_TIZEN)].prog;
     }
   else
     {
#endif
   ...
   pnum = gc->pipe[pn].array.num;
   nv = pnum * 3; nc = pnum * 4; nu = pnum * 2; nu2 = pnum * 2, ns = pnum * 2;
   nt = pnum * 4;
   gc->pipe[pn].array.num += 6;
   array_alloc(gc, pn);
...
#warning Nash: FIXME: Need two shaders?
   ...
   gc->pipe[pn].array.use_texsam = 0;
   gc->pipe[pn].array.texm_smooth = 0;
...

{
   ...
   gc->pipe[pn].array.texm_smooth = EINA_TRUE;
        ...
        if (gc->pipe[i].array.line)
	     ...
	     if (gc->pipe[i].array.use_texm)
		  ...
		  glEnableVertexAttribArray(SHAD_TEXM);
                  ...
                  GLERR(__FUNCTION__, __FILE__, __LINE__, "");
                  if (gc->pipe[i].array.texm_smooth && gc->pipe[i].shader.smooth)
                    {
                       glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
                       GLERR(__FUNCTION__, __FILE__, __LINE__, "");
                       glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
