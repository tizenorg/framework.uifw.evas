...
_surface_buffers_destroy(void *eng_data, EVGL_Surface *sfc)
{
   if (sfc->color_buf)
     {
        if ((sfc->native_buf) && (sfc->native_img))
           evgl_engine->funcs->native_buffer_image_destroy(eng_data, sfc->native_img);
        ...
        if (color_bit & evgl_engine->caps.fbo_fmts[i].color_bit)
          {
             ...
             /* FIXME: Always allow direct rendering here, even if stencil/depth
              * buffers are requested. In theory, we should reconfigure the canvas
              * backbuffer.
              * But evas_x_main.c ALREADY contains a hack to enable both of those
              * buffers during the window surface creation. So it's safe to enable
              * DR whenever it is requested.
              * This is HACK for this Tizen branch only. Please fix asap,
              * by implementing surface reconfigure.
              */
             /* FIXME: Disallow direct rendering when multisample buffers are requested.
              */
             if ((evgl_engine->direct_override) || (!sfc->msaa_samples))
        ...
        if (rsc->direct.map_tex)
          {
             ...
             _texture_attach_2d(rsc->direct.map_tex, GL_COLOR_ATTACHMENT0, 0, sfc->msaa_samples);
