   ...
   if (glsym_eglCreateImage)
     {
        ...
        if (!surface)
          {
             ERR("eglCreateImage() for 0x%x failed", (unsigned int)native);

             if (eglGetError() != EGL_SUCCESS)
                ERR("eglCreateImage failed: %x.", eglGetError());
...
gl_extn_veto(Render_Engine *re)
{
   ...
   if (strstr(vendor, "Qualcomm")) return;
...
# endif
    ...
    {
      bufferid = n->buffer;
      eina_hash_del(re->win->gl_context->shared->native_buffer_hash, &bufferid, im);
      if (n->egl_surface)
        {
          if (glsym_eglDestroyImage)
            {
...
eng_image_native_set(void *data, void *image, void *native)
{
  ...
  Pixmap pm = 0;
  Native *n = NULL;
  uint32_t pmid, texid, bufferid;
  unsigned int tex = 0;
  unsigned int fbo = 0;
#ifdef HAVE_NATIVE_BUFFER
  native_buffer_t *buffer = NULL;
#endif
      ...
      if (ns->type == EVAS_NATIVE_SURFACE_X11)
      ...
      else if (ns->type == EVAS_NATIVE_SURFACE_TIZEN)
        {
           ...
           if (im->native.data)
             {
                Evas_Native_Surface *ens = im->native.data;
                if (ens->data.tizen.buffer == buffer)
                  return im;
             }
        }
#endif
  ...
  if (ns->type == EVAS_NATIVE_SURFACE_X11)
    ...
    {
       bufferid = buffer;
       im2 = eina_hash_find(re->win->gl_context->shared->native_buffer_hash, &bufferid);
       if (im2 == im) return im;
       if (im2)
         {
            n = im2->native.data;
          ...
          if (n)
            {
              ...
              vendor = (const char *)glGetString(GL_VENDOR);
              if (strstr (vendor, "Qualcomm") != NULL)
                n->recreate = 1;
            ...
            if (n)
              {
                 ...
                 eina_hash_add(re->win->gl_context->shared->native_buffer_hash, &bufferid, im);
...
static void *
eng_gl_proc_address_get(void *data __UNUSED__, Evas_GL_Ext *ext, const char *name)
{
   int i = 0;
